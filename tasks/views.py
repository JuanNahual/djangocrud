...
@login_required
def tasks(request):
	tasks = Taks.objects.filter(user=request.user, datecompleted__isnull=True)
...
def create_task(request):
	...
	if request.method == 'GET':
		...
		try:
			...
			return render(request,'created_task.html',{
			'form' : TaskForm,
			'error': "Verifique que los datos ingresados sean validos"})
...
def tasks_detail(request,task_id):
	if request.method =='GET':
		...
		task = get_object_or_404(Taks, pk=task_id, user=request.user)
		...
		try:
			...
			return render(request,'tasks_detail.html',{'task':task,'form':form,'error':"Error al actualizar"})
